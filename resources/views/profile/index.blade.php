...
<style>
    ...
    .dark-theme {
        ...
        --rinsa-bg:           #10100E;
        --rinsa-cream:        #242422;
        --rinsa-cream-dark:   #2E2E2A;
        --rinsa-border:       #3A3A36;
        --rinsa-border-light: #2A2A27;
        --rinsa-dark:         #FDFCFA;
        --rinsa-gray:         #A8A7A2;
        --rinsa-gray-light:   #8C8B86;
        --rinsa-green-pale:   #122A22;
        --rinsa-gold-pale:    #2F2416;
    ...
    .rinsa-dark-toggle:hover {
        transform: scale(1.1) translateY(-2px);
        background: var(--rinsa-green-light);
        box-shadow: 0 6px 20px rgba(15,110,86,0.4);
    }

    /* ===== Interactive Duty Status ===== */
    .interactive-duty-badge {
        cursor: pointer;
        transition: all var(--transition);
        user-select: none;
    }
    .interactive-duty-badge:hover {
    ...
    .notes-paper {
        background: linear-gradient(135deg, var(--rinsa-gold-pale) 0%, var(--rinsa-white) 100%);
        border: 1px solid var(--rinsa-border);
        border-left: 5px solid var(--rinsa-gold);
        border-radius: 16px;
        padding: 1.5rem;
    ...
    .notes-textarea {
        ...
        line-height: 1.6;
        outline: none;
        border-bottom: 1px dashed var(--rinsa-border);
        padding-bottom: 12px;
        margin-bottom: 12px;
    }
    ...
    .spinner {
        ...
        border-top-color: var(--rinsa-green);
        border-radius: 50%;
    ...
    @media print {
        ...
        .nav, .sidebar, .mobile-nav, .rinsa-dark-toggle, .rinsa-toast, .avatar-actions, .header-actions {
        ...
        .quick-notes-section, .activity-section {
            display: none !important;
        }
        * {
    ...
    <div class="profile-card">
        ...
        <div class="profile-header-bg">
            ...
            <div class="header-actions">
                ...
                <div class="status-badge-top interactive-duty-badge" onclick="toggleDutyStatus()" title="Klik untuk mengubah status kehadiran Anda">
                    <span class="status-dot {{ auth()->user()->is_on_duty ? 'online' : 'offline' }}" id="js-duty-dot" aria-hidden="true"></span>
                    <span id="js-duty-text">{{ auth()->user()->is_on_duty ? 'On Duty (Aktif)' : 'Off Duty (Istirahat)' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="profile-content">
            ...
            <div class="avatar-section">
                <div class="avatar-wrapper">
                    @php
                        $__rinsaProfileAvatar = $user->avatar ?? null;
                        $__rinsaProfileHasAvatar = $__rinsaProfileAvatar && \Illuminate\Support\Facades\Storage::disk('public')->exists($__rinsaProfileAvatar);
                    @endphp
                    @if($__rinsaProfileHasAvatar)
                        <img src="{{ asset('storage/' . $user->avatar) }}"
                             class="avatar-img"
                             alt="Foto profil {{ $user->name }}">
                    @else
                        <div class="avatar-initial" aria-hidden="true">{{ substr($user->name, 0, 1) }}</div>
                    @endif
                ...
                <div class="avatar-actions">
                    ...
                    <button class="btn-secondary" onclick="rinsaPrintProfile()" title="Ekspor ringkasan kinerja ke PDF atau Cetak">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" width="14" height="14" style="margin-right: 4px;">
            ...
            <div class="profile-identity">
                ...
                <div class="profile-meta">
                    <span class="user-role-badge">{{ ucfirst($user->role) }} Management</span>
            ...
            <div class="stats-row" aria-label="Ringkasan statistik">
                <div class="stat-cell">
                    ...
                    <div class="stat-number js-counter" data-target="{{ $totalOrders }}">0</div>
                    <div class="stat-label">Total Orders</div>
                </div>
                <div class="stat-cell">
                    <span class="stat-icon">✅</span>
                    <div class="stat-number js-counter" data-target="{{ $completionRate }}" data-suffix="%">0%</div>
                    <div class="stat-label">Completion Rate</div>
                </div>
                <div class="stat-cell">
                    <span class="stat-icon">📈</span>
                    <div class="stat-number js-counter" data-target="{{ $ordersThisMonth ?? 0 }}">0</div>
            ...
            <div class="content-grid">
                ...
                <div>
                    ...
                    <div class="bio-box">
                        {{ $user->bio ?? 'Dedicated administrator focused on providing high-quality management for Rinsa Laundry services.' }}
                    </div>
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-text">
                            ...
                            <div class="info-value">{{ $totalOrders }} Orders</div>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div class="skill-item">
                        <div class="skill-header">
                            ...
                            <span class="skill-pct">{{ $completionRate }}%</span>
                        </div>
                        <div class="skill-bar-bg">
                            <div class="skill-bar-fill" data-width="{{ $completionRate }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="skill-item">
                        <div class="skill-header">
                            ...
                            <span class="skill-pct">{{ $satisfaction }}%</span>
                        </div>
                        <div class="skill-bar-bg">
                            <div class="skill-bar-fill" data-width="{{ $satisfaction }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="skill-item">
                        <div class="skill-header">
                            ...
                            <span class="skill-pct">{{ $systemAccuracy }}%</span>
                        </div>
                        <div class="skill-bar-bg">
                            <div class="skill-bar-fill" data-width="{{ $systemAccuracy }}%"></div>
                        </div>
                    </div>
            </div>
            ...
            <div class="quick-notes-section">
                <h2 class="section-title">Memo & Catatan Cepat Staf</h2>
                <div class="notes-paper">
                    <textarea id="js-notes-area" class="notes-textarea" placeholder="Tulis catatan penting di sini... (Catatan akan otomatis tersimpan saat Anda berhenti mengetik atau klik Simpan)" rows="4">{{ auth()->user()->quick_notes }}</textarea>
                    <div class="notes-footer">
                        <span class="notes-status-text" id="js-notes-status"></span>
                        <button class="btn-secondary btn-notes-save" onclick="rinsaSaveNotes()" id="js-notes-btn">
                            <span id="js-notes-btn-label">💾 Simpan Catatan</span>
                            <span class="spinner" id="js-notes-spinner" style="display:none"></span>
                        </button>
                    </div>
                </div>
            </div>
...
<button class="rinsa-dark-toggle" id="js-dark-toggle" onclick="rinsaToggleDarkMode()" aria-label="Toggle Dark Mode" title="Aktifkan/Nonaktifkan Dark Mode khusus profil">
    <span id="js-dark-icon">🌙</span>
</button>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var darkIcon = document.getElementById('js-dark-icon');
    ...
    window.rinsaToggleDarkMode = function () {
        if (container.classList.contains('dark-theme')) {
            ...
            rinsaShowToast('Tema terang aktif');
        } else {
            container.classList.add('dark-theme');
            localStorage.setItem('profile-theme', 'dark');
            if (darkIcon) darkIcon.textContent = '☀️';
            rinsaShowToast('Tema gelap premium aktif');
        }
    };

    /* ── 7. Duty Status Toggle (AJAX) ── */
    var isTogglingDuty = false;
    window.toggleDutyStatus = function () {
        if (isTogglingDuty) return;
        isTogglingDuty = true;
        
        var dot = document.getElementById('js-duty-dot');
        var text = document.getElementById('js-duty-text');
        var currentIsOnDuty = !dot.classList.contains('offline');
        var nextIsOnDuty = !currentIsOnDuty;

        var token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        
        fetch('{{ route("profile.duty") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': token
            },
            body: JSON.stringify({ is_on_duty: nextIsOnDuty ? 1 : 0 })
        })
        ...
        .then(function (data) {
            if (data.success) {
                if (data.is_on_duty) {
                    ...
                    text.textContent = 'On Duty (Aktif)';
                } else {
                    dot.className = 'status-dot offline';
                    text.textContent = 'Off Duty (Istirahat)';
                }
                rinsaShowToast(data.message);
            } else {
                rinsaShowToast('Gagal memperbarui status');
            }
        })
        .catch(function () {
            rinsaShowToast('Kesalahan koneksi server');
        })
        .finally(function () {
    ...
    /* ── 8. Quick Notes Logic (AJAX + Autosave) ── */
    var notesArea = document.getElementById('js-notes-area');
    var notesStatus = document.getElementById('js-notes-status');
    var notesBtnLabel = document.getElementById('js-notes-btn-label');
    var notesSpinner = document.getElementById('js-notes-spinner');
    var saveTimeout = null;

    window.rinsaSaveNotes = function () {
        if (!notesArea) return;
        
        // Show saving UI
        if (notesSpinner) notesSpinner.style.display = 'inline-block';
        if (notesBtnLabel) notesBtnLabel.textContent = ' Menyimpan...';
        if (notesStatus) notesStatus.textContent = 'Menyimpan...';

        var token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        var notesValue = notesArea.value;

        fetch('{{ route("profile.notes") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': token
            },
            body: JSON.stringify({ quick_notes: notesValue })
        })
        .then(function (res) { return res.json(); })
        .then(function (data) {
            if (data.success) {
                rinsaShowToast('Catatan disimpan!');
                if (notesStatus) {
                    notesStatus.textContent = '✓ Catatan berhasil disimpan';
                    setTimeout(function() { notesStatus.textContent = ''; }, 3000);
                }
            } else {
                rinsaShowToast('Gagal menyimpan catatan');
            }
        })
        .catch(function () {
            rinsaShowToast('Gagal menghubungi server');
        })
        .finally(function () {
            if (notesSpinner) notesSpinner.style.display = 'none';
            if (notesBtnLabel) notesBtnLabel.textContent = '💾 Simpan Catatan';
        });
    };

    // Autosave feature (debounced input)
    if (notesArea) {
        notesArea.addEventListener('input', function () {
            if (notesStatus) notesStatus.textContent = 'Mengetik...';
            clearTimeout(saveTimeout);
            saveTimeout = setTimeout(function () {
                if (notesStatus) notesStatus.textContent = 'Menyimpan otomatis...';
                ...
                .then(function (data) {
                    if (data.success && notesStatus) {
                        notesStatus.textContent = '✓ Tersimpan otomatis';
                        setTimeout(function() { if (notesStatus.textContent === '✓ Tersimpan otomatis') notesStatus.textContent = ''; }, 3000);
                    }
                })
                .catch(function () {
                    if (notesStatus) notesStatus.textContent = '⚠️ Gagal autosave';
                });
            }, 1500); // 1.5s debounce
        });
    }
    ...
    /* ── 9. Print / PDF Export ── */
    window.rinsaPrintProfile = function () {
        rinsaShowToast('Menyiapkan halaman cetak...');
        setTimeout(function () {
            window.print();
        }, 300);
    };
